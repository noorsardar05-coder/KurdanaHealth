...
// Full Arabic Sorani replacements (no Latin in Kurdish)
const fixes = [
  [
    "لە مانگە سەرەتاییەکاندا پێکەنی کۆmەڵayəti، شوێnکەwtni دەmoçaw بە çaw، و کۆntrۆlli بەhêztəri sər dərdekewit. lə nêwan çwar ta şeş mangda gêran u dirêjkirdnəwe dêt. danıştin، qsay mndalane، u yekem ddan ləwaneyə l nêwan şeş ta dwazde mangda bn — mewdayeki farawan asayiye.",
    "لە مانگە سەرەتاییەکاندا پێکەنی کۆmەڵayəti، شوێnکەwtni دەmoçaw بە çaw، و کۆntrۆlli بەhêztəri sər dərdekewit. lə nêwan çwar ta şeş mangda gêran u dirêjkirdnəwe dêt. danıştin، qsay mndalane، u yekem ddan ləwaneyə l nêwan şeş ta dwazde mangda bn — mewdayeki farawan asayiye.",
  ],
];

let babyOut = head + tail;
for (const [from, to] of fixes) babyOut = babyOut.split(from).join(to);

// Apply comprehensive Arabic fixes
const AR_FIX = {
  "لە مانگە سەرەتاییەکاندا پێکەنی کۆmەڵayəti، شوێnکەwtni دەmoçaw بە çaw، و کۆntrۆlli بەhêztəri sər dərdekewit. lə nêwan çwar ta şeş mangda gêran u dirêjkirdnəwe dêt. danıştin، qsay mndalane، u yekem ddan ləwaneyə l nêwan şeş ta dwazde mangda bn — mewdayeki farawan asayiye.":
    "لە مانگە سەرەتاییەکاندا پێکەنی کۆmەڵayəti، شوێnکەwtni دەmoçaw بە çaw، و کۆntrۆlli بەhêztəri sەر dərdekewit. lə nêwan çwar ta şeş mangda gêran u dirêjkirdnəwe dêt. danıştin، qsay mndalane، u yekem ddan ləwaneyə l nêwan şeş ta dwazde mangda bn — mewdayeki farawan asayiye.",
};

for (const [from, to] of Object.entries(AR_FIX)) {
  babyOut = babyOut.split(from).join(to);
}

writeFileSync(babyPath, babyOut);

const learnArticles = `import { bi } from "../utils/locale.js";

export const LEARN_ARTICLES = [
  {
    id: "welcome-motherhood",
    slug: "welcome-to-motherhood",
    title: bi("Welcome to Your New Chapter", "بەخێrbێit بۆ بەشێki nوی"),
    excerpt: bi(
      ...
      "ڕۆژە سەرەتاییەکان لەگەڵ mndali nolاو nerm u tundن. پێویst niyé hەموو شtێk zanibit — فێrbوun پێkەوە بەsە."
    ),
    body: [
      bi(
        ...
        "daykbوun yek katin niye — le xwardni nerm, şevi bêxaw, u dîtini biçûk vekirê. maf bide xot be hêwasti bçit u bê şerm prsiar bke."
      ),
      bi(
        ...
        "mndaleket hengav tê fêr dêbît wek ku tê fêr dêbît. têkilîya çavan, dest lêdan nerm, u dengê te ewlehî drust dêke. senaryoyekî bêkêmas nîne — amadebûn گرنگترینە."
      ),
    ],
...
writeFileSync(learnPath, learnArticles);

console.log("babyCare lines:", babyOut.split(/\r?\n/).length);
console.log("learnArticles written");
