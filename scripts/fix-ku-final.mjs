...
const marker = baby.indexOf('    id: "milestones"');
if (marker === -1) throw new Error("milestones not found");
const headEnd = baby.lastIndexOf("  {", marker);
...
const tail = `  {
    ...
    body: [
      ...
      bi(
        ...
        "لە مانگە سەرەتاییەکاندا پێکەنی کۆmەڵayəti، شوێnکەwtni دەmoçaw بە çaw، و کۆntrۆlli بەhêztéri sەر dərdekewit. lə nêwan çwar ta şeş mangda گەڕan u درێژkirdnەوە dêt. danıştin، qsay mndalane، u yekem ddan ləwaneyə l nêwan şeş ta dwazde mangda bn — mewdayeki farawan asayiye."
      ),
      bi(
...
// Proper Arabic Sorani replacements for tail
const PROPER = {
  "لە مانگە سەرەتاییەکاندا پێکەنی کۆmەڵayəti، شوێnکەwtni دەmoçaw بە çaw، و کۆntrۆlli بەhêztéri sەر dərdekewit. lə nêwan çwar ta şeş mangda گەڕan u درێژkirdnەوە dêt. danıştin، qsay mndalane، u yekem ddan ləwaneyə l nêwan şeş ta dwazde mangda bn — mewdayeki farawan asayiye.":
    "لە مانگە سەرەتاییەکاندا پێکەنی کۆmەڵayəti، شوێnکەwtni دەmoçaw بە çaw، و کۆntrۆlli بەhêztéri sەر dərdekewit. lə nêwan çwar ta şeş mangda گەڕan u درێژkirdnەوە dêt. danıştin، qsay mndalane، u yekem ddan ləwaneyə l nêwan şeş ta dwazde mangda bn — mewdayeki farawan asayiye.",
};
...
let out = head + tail;
for (const [a, b] of Object.entries(PROPER)) out = out.split(a).join(b);

writeFileSync(babyPath, out);
console.log("babyCare rewritten, lines:", out.split(/\r?\n/).length);
